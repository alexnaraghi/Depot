...
- Keep changelist and revision as placeholders for now
  ...
  <verify>`npx tsc --noEmit` passes. Run `npm run dev`, verify WorkspaceSummaryView shows on launch in the center pane. Verify FileDetailView renders by checking the component exists and DetailPane routes `type: 'file'` to it.</verify>
