...
- `reconcileApply` — TanStack useMutation calling invokeP4ReconcileApply with selected file paths. On success: toast success ("N file(s) reconciled"), invalidate `['p4', 'opened']`, `['p4', 'changes']`, `['p4', 'fstat']`.
...
- On Apply:
  ...
  2. Call reconcileApply.mutateAsync with Array.from(selectedPaths)
...
- Apply reconcile processes only selected files
