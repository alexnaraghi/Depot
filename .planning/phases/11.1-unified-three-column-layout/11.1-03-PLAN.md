...
- This requires fetching the changelist details for `revision.change`. Use TanStack Query: `useQuery({ queryKey: ['p4', 'describe', revision.change], queryFn: ... })`.
- Check if `invokeP4Describe` or similar exists in tauri.ts. If not, show a TODO placeholder: "Sibling files not yet available (needs p4 describe backend)".
- If available, show file list. Each file is clickable — drill into that file's detail.
