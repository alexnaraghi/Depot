...
First, add AlertDialog component if not present:
```bash
...
- Error handling via toast from hook
  ...
  <verify>TypeScript compiles: `npx tsc --noEmit src/components/ChangelistPanel/SubmitDialog.tsx`</verify>
...
export function ChangelistPanel({ className }: ChangelistPanelProps) {
  ...
  const handleMove: MoveHandler<ChangelistTreeNode> = useCallback(async ({ dragIds, parentId }) => {
    ...
    try {
      ...
      // Note: This is p4 reopen -c <changelist> <files>
      // We'll use invokeP4Edit with existing files (it handles reopen)
