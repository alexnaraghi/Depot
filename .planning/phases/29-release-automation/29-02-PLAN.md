...
<task type="auto">
  ...
  <files></files>
  <action>
Set up remote (if needed) and push the workflow file and CONTRIBUTING.md updates to GitHub.

**Git operations:**
1. Check if remote origin exists: `git remote -v`
2. If no origin exists, add it: `git remote add origin https://github.com/alexnaraghi/Depot.git`
3. Stage the new/modified files:
   - `.github/workflows/publish.yml`
   - `CONTRIBUTING.md`
4. Commit with message: `ci(29): add release automation workflow`
...
If push fails (e.g., diverged history, authentication issues), note this for the checkpoint task.
  </action>
  <verify>
1. Remote configured: `git remote -v | grep origin`
2. Git log shows commit: `git log -1 --oneline`
3. Push successful: no error output from `git push`
4. Or note failure reason for checkpoint
  </verify>
  <done>
- Remote origin configured to https://github.com/alexnaraghi/Depot.git
...
1. Go to https://github.com/alexnaraghi/Depot
