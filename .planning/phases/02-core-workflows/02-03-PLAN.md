...
**Note on reopen:** When `p4_edit` is called with files that are already opened, Perforce treats this as a reopen operation (moving files between changelists). The `-c <changelist>` flag specifies the target changelist. This means a separate `p4_reopen` command is not needed - `p4_edit` handles both initial checkout AND moving files between changelists.
