...
must_haves:
  truths:
    ...
    - "Command isolates errors per changelist (one failure doesn't fail batch)"
...
6. Handle stderr:
   - "no shelved files" messages = empty result for that CL (not error)
   - Actual errors logged but don't fail batch
