...
must_haves:
  truths:
    - "Shelve/unshelve/delete-shelf/describe-shelved operations can be invoked from the frontend"
    - "Reconcile preview and apply operations can be invoked from the frontend"
    - "Backend returns structured data for shelved files (depot path, action, type, revision)"
    - "Backend returns structured data for reconcile preview (path, action)"
  artifacts:
...
6. **p4_reconcile_apply(file_paths: Vec<String>, changelist_id: Option<i32>, server, user, client)** → Result<String, String>
   - If changelist_id is Some: runs `p4 reconcile -c <cl> <file_paths...>`
   - If changelist_id is None: runs `p4 reconcile <file_paths...>` (uses default changelist)
...
6. **invokeP4ReconcileApply(filePaths: string[], changelistId?: number, server?, user?, client?)** → Promise<string>
   - Invokes "p4_reconcile_apply" with file_paths and optional changelist_id
